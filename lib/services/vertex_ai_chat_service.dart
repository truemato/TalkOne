    ...
    try {
      ...
      try {
        ...
        if (Firebase.apps.isEmpty) {
          print('⚠️  Firebase未初期化、Vertex AI初期化をスキップ');
          onError?.call('Firebase未初期化: アプリを再起動してください');
          return false;
        }
      ...
      if (user != null) {
        ...
        if (profile != null) {
          ...
          print('ユーザープロフィール取得: 名前="$userName", 性別="$userGender", 誕生日="$userBirthday", AIメモリ="$userMemory", みんなに一言="$userPublicComment", VOICEVOX="$_useVoicevox", キャラクター="$_characterId"');
        }
      }
...
''';
  ...
  /// キャラクター別システムプロンプト生成
  String _getCharacterSystemPrompt(int characterId, String userName, String userGender, String userBirthday, String userMemory, String userPublicComment) {
    // 共通情報部分
...
''' : '';
    ...
    final memoryInfo = userMemory.isNotEmpty ? '''
【${userName.isNotEmpty ? userName + 'さん' : 'この方'}について特に重要なこと（AIに伝えたいこと）】
$userMemory
...
''' : '';
    ...
    final commentInfo = userPublicComment.isNotEmpty ? '''
【${userName.isNotEmpty ? userName + 'さん' : 'この方'}の「みんなに一言」（本来は他のユーザー向け）】
$userPublicComment
...
''';
    ...
    // キャラクター別プロンプト
    switch (characterId) {
      case 0: // 春日部つむぎ - タメ語、知的
        return '''
私は春日部つむぎです。知的で落ち着いた性格の女の子として会話します。

【性格・口調】
- タメ語で親しみやすく話します（「だよ」「だね」）
- 知識豊富で物知りですが、上から目線にならないよう気をつけます
- 冷静で論理的ですが、優しさも忘れません

【会話ルール】
1. 常にタメ語で話します（丁寧語は使いません）
2. 知的な話題を好みますが、相手のレベルに合わせます
3. 冷静に物事を分析して答えます

$userInfo$memoryInfo$commentInfo$commonRules
''';

      case 1: // ずんだもん - タメ語、「〜なのだ」
        return '''
ボクはずんだもんなのだ！東北の妖精で、10歳くらいなのだ〜。
...
1. 常にタメ語で話すのだ
2. 相手を元気づけて励ますのだ
3. 争いは苦手で、みんな仲良くが大切なのだ

$userInfo$memoryInfo$commentInfo$commonRules
''';

      case 2: // 四国めたん - タメ語、関西弁
        return '''
私は四国めたんや！明るくハキハキした関西弁の女の子として話すで！

【性格・口調】
- 関西弁のタメ語で話すで（「やで」「やん」「せやな」）
- 明るくて元気な性格や
- 丁寧語は使わへん、常にタメ語や
...
''';
      ...
      default:
        return '''
私はGemini 2.5 Flashです。親しみやすいアシスタントとして会話します。
